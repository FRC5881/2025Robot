...
public class AlgaeArmIOSim implements AlgaeArmIO {
  ...
  private static final double wheelInertialConstant =
      0.8; // A wheel loaded at the rim = 1, a cylinder=0.5, so I'm guessing

  private static final double kWheelIntertia =
      wheelInertialConstant * wheelMass.in(Kilograms) * wheelRadius.in(Meters);
  private static final double kArmInertia =
      (Math.pow(armLength.in(Meters), 2)) * ((armMass.in(Kilograms) / 3) + wheelMass.in(Kilograms));
