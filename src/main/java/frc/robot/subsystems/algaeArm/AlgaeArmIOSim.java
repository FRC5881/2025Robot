...
public class AlgaeArmIOSim implements AlgaeArmIO {
    ...
    private static final double wheelInertialConstant = 0.8; // A wheel loaded at the rim = 1, a cylinder=0.5, so I'm
                                                             // guessing

    private static final double kWheelIntertia = wheelInertialConstant * wheelMass.in(Kilograms)
            * wheelRadius.in(Meters);
    private static final double kArmInertia = (Math.pow(armLength.in(Meters), 2))
            * ((armMass.in(Kilograms) / 3) + wheelMass.in(Kilograms));

    SingleJointedArmSim pivotSim = new SingleJointedArmSim(DCMotor.getNEO(1), 10, kArmInertia, armLength.in(Meters),
            -Math.PI / 2, 3 * Math.PI / 4, true, -Math.PI / 2);
    FlywheelSim intakeSim = new FlywheelSim(
            LinearSystemId.createFlywheelSystem(DCMotor.getNeo550(1), kWheelIntertia, 3),
            DCMotor.getNeo550(1));
