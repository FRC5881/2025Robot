...
public class AlgaeArmSubsystem extends SubsystemBase {
  ...
  private final PIDController pivotPID =
      new PIDController(0.15, 0.1, 0.05); // This is okay but needs more tuning
  ...
  public double algaeIntakeSetpoint = 0;
  ...
  public Command cPivotDegControl() {
    return runEnd(
        () -> {
          ...
          io.setArmVoltages(
              Optional.of(
                  pivotFF.calculate(this.pivotSetpoint.getDegrees())
                      + pivotPID.calculate(
                          currentPos.getDegrees(), this.pivotSetpoint.getDegrees())),
              Optional.empty());
        },
        io::stop);
  }
    ...
    return runEnd(
        () -> {
          ...
          io.setArmVoltages(
              Optional.empty(),
              Optional.of(
                  intakeFF.calculate(algaeIntakeSetpoint)
                      + intakePID.calculate(currentSpeed, algaeIntakeSetpoint)));
        },
        io::stop);
  }
