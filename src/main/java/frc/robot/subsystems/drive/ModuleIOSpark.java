...
//   private final TrapezoidProfile turnProfile = new TrapezoidProfile(turnConstraints);
  ...
  public ModuleIOSpark(int module) {
    ...
    turnPositionQueue =
        SparkOdometryThread.getInstance().registerSignal(turnSpark.getAnalog()::getPosition);

    turnPidController.enableContinuousInput(0, 2 * Math.PI);
    SmartDashboard.putData("Turn PID", turnPidController);
  }
  ...
  public void updateInputs(ModuleIOInputs inputs) {
    ...
    ifOk(
        turnSpark,
        turnSpark.getAnalog()::getPosition,
        (value) ->
            inputs.turnPosition = new Rotation2d((value / 3.3) * 2 * Math.PI).minus(zeroRotation));
    ifOk(
        turnSpark,
        turnSpark.getAnalog()::getVelocity,
    ...
    inputs.odometryTurnPositions =
        turnPositionQueue.stream()
            .map((Double value) -> new Rotation2d((value / 3.3) * 2 * Math.PI).minus(zeroRotation))
