...
public class RobotContainer {
    ...
    private void configureBindings() {
        drive.setDefaultCommand(
                DriveCommands.twoPlayerDrive(drive, m_driverController.getHID(), m_copilotController.getHID()));

        // Reset gyro / odometry
        m_driverController
                .touchpad().or(m_copilotController.touchpad())
                .onTrue(
                        Commands.runOnce(
                                () -> drive.resetOdometry(
        ...
        l2Subsystem.reefTrigger()
                .and(this::anyBumper)
                .onTrue(
                        new ParallelCommandGroup(rumbleCommand(), DriveCommands.stopWithX(drive),
                                l2Subsystem.outputLeftCommand(), leds.cSetPattern(RawPattern.FAST_RAINBOW_FLASH))
                                .withTimeout(1));
    }

    private boolean anyBumper() {
        return m_driverController.L1().getAsBoolean() || m_driverController.R1().getAsBoolean() ||
                m_copilotController.L1().getAsBoolean() || m_copilotController.R1().getAsBoolean();
    }
